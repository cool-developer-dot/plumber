...
import {
  DRAIN_CLEANING_HERO,
  DRAIN_CLEANING_IMAGES,
  DRAIN_CLEANING_PROCESS_STEPS,
  DRAIN_CLEANING_RELATED,
  DRAIN_CLEANING_SECTIONS,
  DRAIN_CLEANING_SERVICE_AREAS,
  DRAIN_CLEANING_SERVICES,
  DRAIN_CLEANING_URGENCY,
  DRAIN_CLEANING_WHY_CHOOSE,
} from "@/lib/drain-cleaning-content";
...
function ServiceHero() {
  return (
    <section
      ...
      aria-labelledby="drain-cleaning-heading"
    >
      <div
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16 xl:gap-20">
          ...
          >
            ...
            <ul className="mt-8 flex flex-wrap gap-2.5" aria-label="Trust badges">
              {DRAIN_CLEANING_HERO.trustBadges.map((badge) => (
                <li
                  key={badge}
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[rgba(21,23,27,0.08)] bg-soft-white shadow-[0_20px_50px_-12px_rgba(21,23,27,0.12)]">
              <Image
                src={DRAIN_CLEANING_IMAGES.hero.src}
                alt={DRAIN_CLEANING_IMAGES.hero.alt}
                fill
                priority
              ...
              <div className="flex items-center gap-2.5 rounded-xl border border-[rgba(21,23,27,0.08)] bg-white px-4 py-3 shadow-[0_8px_30px_rgba(21,23,27,0.08)]">
                ...
                <div className="leading-tight">
                  <p className="text-[0.9rem] font-bold text-deep-charcoal">
                    {DRAIN_CLEANING_HERO.responseBadge.title}
                  </p>
                  <p className="text-[0.72rem] text-cool-gray">
                    {DRAIN_CLEANING_HERO.responseBadge.subtitle}
                  </p>
                </div>
              </div>
            </div>
...
function DrainServicesSection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.situations;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="drain-services-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          ...
          headingId="drain-services-heading"
        />
        ...
        >
          {DRAIN_CLEANING_SERVICES.map((situation, index) => (
            <SituationCard
              key={situation.title}
              ...
              href={situation.href}
              icon={SITUATION_ICONS[index] ?? Waves}
            />
          ))}
        </motion.div>
      </div>
...
function WhyChooseSection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.whyChoose;

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="why-drain-cleaning-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="why-drain-cleaning-heading"
        />
        ...
        <div className="mt-14 grid items-start gap-10 lg:mt-16 lg:grid-cols-2 lg:gap-16">
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[rgba(21,23,27,0.08)] shadow-[0_12px_40px_rgba(21,23,27,0.08)]">
              <Image
                src={DRAIN_CLEANING_IMAGES.team.src}
                alt={DRAIN_CLEANING_IMAGES.team.alt}
                fill
                sizes="(max-width: 1024px) 100vw, 50vw"
                loading="lazy"
                quality={80}
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-60px" }}
          >
            {DRAIN_CLEANING_WHY_CHOOSE.map((item, index) => (
              <FeatureCard
                key={item.title}
                title={item.title}
...
function RelatedServicesSection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.solutions;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="drain-related-services-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="drain-related-services-heading"
        />
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {DRAIN_CLEANING_RELATED.map((solution, index) => (
            <SolutionCard
              key={solution.title}
...
function ProcessStep({
  ...
  return (
    ...
    >
      ...
      {index < DRAIN_CLEANING_PROCESS_STEPS.length - 1 ? (
        <div
...
function DrainProcessSection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.process;
  const stepIcons = [CalendarCheck, ScanSearch, CircleCheck];

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="drain-process-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="drain-process-heading"
        />
        ...
        <div className="relative mt-14 sm:mt-16 lg:mt-20">
          ...
          <motion.ol
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-60px" }}
            aria-label="How our drain cleaning service works in three steps"
          >
            {DRAIN_CLEANING_PROCESS_STEPS.map((step, index) => (
              <ProcessStep
                key={step.number}
                {...step}
        ...
        >
          <div className="relative aspect-[21/9] min-h-[200px] sm:aspect-[21/8]">
            <Image
              src={DRAIN_CLEANING_IMAGES.arrival.src}
              alt={DRAIN_CLEANING_IMAGES.arrival.alt}
              fill
            ...
            <div className="absolute inset-0 flex items-center px-6 sm:px-10 lg:px-14">
              <p className="max-w-lg text-[0.95rem] leading-relaxed text-soft-white sm:text-[1.05rem]">
                {DRAIN_CLEANING_SECTIONS.processBanner}{" "}
...
function UrgencySection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.urgency;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="drain-urgency-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="drain-urgency-heading"
        />
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {DRAIN_CLEANING_URGENCY.map((risk, index) => {
...
function ServiceAreasSection() {
  const { eyebrow, title, description } = DRAIN_CLEANING_SECTIONS.serviceAreas;

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="drain-service-areas-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="drain-service-areas-heading"
        />
        ...
        <motion.ul
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {DRAIN_CLEANING_SERVICE_AREAS.map((area) => (
...
function FinalCtaSection() {
  const { eyebrow, headline, description } = DRAIN_CLEANING_SECTIONS.finalCta;
        ...
        >
          <div className="grid lg:grid-cols-2">
            <div className="relative min-h-[220px] lg:min-h-full">
              <Image
                src={DRAIN_CLEANING_IMAGES.cta.src}
                alt={DRAIN_CLEANING_IMAGES.cta.alt}
                fill
                sizes="(max-width: 1024px) 100vw, 50vw"
                loading="lazy"
                quality={80}
            ...
            <div className="relative z-10 flex flex-col justify-center px-6 py-12 sm:px-10 sm:py-14 lg:px-12 lg:py-16">
              <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-electric-blue/15 bg-electric-blue/[0.06] px-3.5 py-1.5">
                <Waves className="h-3.5 w-3.5 text-electric-blue" strokeWidth={2} aria-hidden />
                <span className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-electric-blue">
                  {eyebrow}
                </span>
              </div>
