...
import {
  LEAK_REPAIR_HERO,
  LEAK_REPAIR_IMAGES,
  LEAK_REPAIR_PROCESS_STEPS,
  LEAK_REPAIR_RELATED,
  LEAK_REPAIR_SECTIONS,
  LEAK_REPAIR_SERVICE_AREAS,
  LEAK_REPAIR_SERVICES,
  LEAK_REPAIR_URGENCY,
  LEAK_REPAIR_WHY_CHOOSE,
} from "@/lib/leak-repair-content";
...
function ServiceHero() {
  return (
    <section
      ...
      aria-labelledby="leak-repair-heading"
    >
      <div
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16 xl:gap-20">
          ...
          >
            ...
            <ul className="mt-8 flex flex-wrap gap-2.5" aria-label="Trust badges">
              {LEAK_REPAIR_HERO.trustBadges.map((badge) => (
                <li
                  key={badge}
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[rgba(21,23,27,0.08)] bg-soft-white shadow-[0_20px_50px_-12px_rgba(21,23,27,0.12)]">
              <Image
                src={LEAK_REPAIR_IMAGES.hero.src}
                alt={LEAK_REPAIR_IMAGES.hero.alt}
                fill
                priority
              ...
              <div className="flex items-center gap-2.5 rounded-xl border border-[rgba(21,23,27,0.08)] bg-white px-4 py-3 shadow-[0_8px_30px_rgba(21,23,27,0.08)]">
                ...
                <div className="leading-tight">
                  <p className="text-[0.9rem] font-bold text-deep-charcoal">
                    {LEAK_REPAIR_HERO.responseBadge.title}
                  </p>
                  <p className="text-[0.72rem] text-cool-gray">
                    {LEAK_REPAIR_HERO.responseBadge.subtitle}
                  </p>
                </div>
              </div>
            </div>
...
function LeakServicesSection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.situations;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="leak-services-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          ...
          headingId="leak-services-heading"
        />
        ...
        >
          {LEAK_REPAIR_SERVICES.map((situation, index) => (
            <SituationCard
              key={situation.title}
...
function WhyChooseSection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.whyChoose;

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="why-leak-repair-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="why-leak-repair-heading"
        />
        ...
        <div className="mt-14 grid items-start gap-10 lg:mt-16 lg:grid-cols-2 lg:gap-16">
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[rgba(21,23,27,0.08)] shadow-[0_12px_40px_rgba(21,23,27,0.08)]">
              <Image
                src={LEAK_REPAIR_IMAGES.team.src}
                alt={LEAK_REPAIR_IMAGES.team.alt}
                fill
                sizes="(max-width: 1024px) 100vw, 50vw"
                loading="lazy"
                quality={80}
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-60px" }}
          >
            {LEAK_REPAIR_WHY_CHOOSE.map((item, index) => (
              <FeatureCard
                key={item.title}
                title={item.title}
...
function RelatedServicesSection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.solutions;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="leak-related-services-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="leak-related-services-heading"
        />
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {LEAK_REPAIR_RELATED.map((solution, index) => (
            <SolutionCard
              key={solution.title}
...
function ProcessStep({
  ...
  return (
    ...
    >
      ...
      {index < LEAK_REPAIR_PROCESS_STEPS.length - 1 ? (
        <div
...
function LeakProcessSection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.process;
  const stepIcons = [CalendarCheck, ScanSearch, CircleCheck];

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="leak-process-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="leak-process-heading"
        />
        ...
        <div className="relative mt-14 sm:mt-16 lg:mt-20">
          ...
          <motion.ol
            ...
            whileInView="visible"
            viewport={{ once: true, margin: "-60px" }}
            aria-label="How our leak repair service works in three steps"
          >
            {LEAK_REPAIR_PROCESS_STEPS.map((step, index) => (
              <ProcessStep
                key={step.number}
                {...step}
        ...
        >
          <div className="relative aspect-[21/9] min-h-[200px] sm:aspect-[21/8]">
            <Image
              src={LEAK_REPAIR_IMAGES.arrival.src}
              alt={LEAK_REPAIR_IMAGES.arrival.alt}
              fill
            ...
            <div className="absolute inset-0 flex items-center px-6 sm:px-10 lg:px-14">
              <p className="max-w-lg text-[0.95rem] leading-relaxed text-soft-white sm:text-[1.05rem]">
                {LEAK_REPAIR_SECTIONS.processBanner}{" "}
...
function UrgencySection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.urgency;

  return (
    <section
      className="bg-soft-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="leak-urgency-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="leak-urgency-heading"
        />
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {LEAK_REPAIR_URGENCY.map((risk, index) => {
...
function ServiceAreasSection() {
  const { eyebrow, title, description } = LEAK_REPAIR_SECTIONS.serviceAreas;

  return (
    <section
      className="bg-white py-20 sm:py-24 lg:py-28"
      aria-labelledby="leak-service-areas-heading"
    >
      <div className="mx-auto max-w-[1320px] px-5 sm:px-6 lg:px-8">
        <ServiceSectionHeader
          eyebrow={eyebrow}
          title={title}
          description={description}
          headingId="leak-service-areas-heading"
        />
        ...
        <motion.ul
          ...
          viewport={{ once: true, margin: "-60px" }}
        >
          {LEAK_REPAIR_SERVICE_AREAS.map((area) => (
...
function FinalCtaSection() {
  const { eyebrow, headline, description } = LEAK_REPAIR_SECTIONS.finalCta;
        ...
        >
          <div className="grid lg:grid-cols-2">
            <div className="relative min-h-[220px] lg:min-h-full">
              <Image
                src={LEAK_REPAIR_IMAGES.cta.src}
                alt={LEAK_REPAIR_IMAGES.cta.alt}
                fill
                sizes="(max-width: 1024px) 100vw, 50vw"
                loading="lazy"
                quality={80}
            ...
            <div className="relative z-10 flex flex-col justify-center px-6 py-12 sm:px-10 sm:py-14 lg:px-12 lg:py-16">
              <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-electric-blue/15 bg-electric-blue/[0.06] px-3.5 py-1.5">
                <Droplets className="h-3.5 w-3.5 text-electric-blue" strokeWidth={2} aria-hidden />
                <span className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-electric-blue">
                  {eyebrow}
                </span>
              </div>
